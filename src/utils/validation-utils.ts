 ...
 */
export function validateConfiguration(config: any, requiredFields: string[]): ValidationResult {
    ...
    if (config.type && !validateHomelabType(config.type)) {
        result.errors.push('Invalid homelab type. Must be: local, basic-cloud, or advanced-cloud');
        result.isValid = false;
    }
 ...
 */
export function validateConfigurationOrThrow(config: any, requiredFields: string[]): void {
