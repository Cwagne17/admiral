 ...
 */
export function getAvailabilityZones(region: string, count: number = 2): string[] {
...
export function generateSubnetCidrs(
    ...
    if (subnetSize <= vpcMaskNum) {
        throw new Error(`Subnet size /${subnetSize} must be larger than VPC mask /${vpcMaskNum}`);
    }
