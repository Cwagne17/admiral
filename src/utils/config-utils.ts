 ...
 */
export function loadEnvironmentConfig<T>(
  environment: string,
  ...
  try {
    ...
    if (defaultConfig) {
      console.warn(
        `Warning: Could not load config for ${environment}, using defaults`,
      );
...
export function getContextValue(
  ...
  if (value === undefined && defaultValue === undefined) {
    throw new Error(
      `Required context value '${key}' not found. Use -c ${key}=<value>`,
    );
  }
